...
func RequestEpay(c *gin.Context) {
	...
	var quote *CNYTopUpQuote
	if req.PaymentAmountCNY != nil {
		if !isEpayTopUpEnabled() {
			c.JSON(http.StatusForbidden, gin.H{"message": "error", "data": "管理员尚未启用在线充值"})
			return
		}
		if req.Amount != 0 {
			c.JSON(http.StatusOK, gin.H{"message": "error", "data": "不能同时指定人民币金额和旧版充值数量"})
			return
		}
		result, quoteErr := quoteCNYTopUp(*req.PaymentAmountCNY)
		if quoteErr != nil {
			c.JSON(http.StatusOK, gin.H{"message": "error", "data": quoteErr.Error()})
			return
		}
		quote = &result
	} else if req.Amount < getMinTopup() {
	...
	if quote != nil {
		payMoney = quote.PaymentAmountCNY
	} else {
	...
	if quote != nil {
		orderName = fmt.Sprintf("充值 ¥%.2f", quote.PaymentAmountCNY)
	}
	...
	if quote != nil {
		topUp.CreditedQuota = &quote.CreditedQuota
		// Amount is retained only for legacy clients; new clients use the
		// exact credited_quota rather than an integer USD quantity.
		topUp.Amount = int64(common.QuotaFromFloat(quote.CreditedAmountUSD))
	}
	err = topUp.Insert()
	if err != nil {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值订单创建成功 user_id=%d trade_no=%s payment_method=%s money=%.2f", id, tradeNo, req.PaymentMethod, payMoney))
...
func EpayNotify(c *gin.Context) {
	...
	if err != nil || !verifyInfo.VerifyStatus {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 webhook 验签失败 client_ip=%s", c.ClientIP()))
		_, _ = c.Writer.Write([]byte("fail"))
		return
	}
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		topUp, applied, completeErr := model.CompleteEpayTopUp(verifyInfo.ServiceTradeNo, verifyInfo.Money, verifyInfo.Type)
		if completeErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 充值入账失败 trade_no=%s error=%q", verifyInfo.ServiceTradeNo, completeErr.Error()))
			_, _ = c.Writer.Write([]byte("fail"))
			return
		}
		if applied {
			quotaToAdd, _ := topUp.EpayCreditQuota()
			model.RecordTopupLog(topUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(quotaToAdd), topUp.Money), c.ClientIP(), topUp.PaymentMethod, "epay")
		}
	}
	// Only acknowledge after the transaction commits, so failed credits can retry.
...
func RequestAmount(c *gin.Context) {
	...
	if req.PaymentAmountCNY != nil {
		...
		quote, quoteErr := quoteCNYTopUp(*req.PaymentAmountCNY)
		if quoteErr != nil {
			c.JSON(http.StatusOK, gin.H{"message": "error", "data": quoteErr.Error()})
			return
		}
