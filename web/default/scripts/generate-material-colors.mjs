...
function renderRootBlock(isDark) {
  ...
  const workspaceSchemes = Object.fromEntries(
    WORKSPACES.map((workspace) => {
      const scheme = createScheme(workspace.seed, isDark)
      validateScheme(
  ...
  const statusSchemes = Object.fromEntries(
    WORKSPACES.map((workspace) => {
      const schemes = createStatusSchemes(workspace.seed, isDark)
      validateStatusSchemes(
 ...
 * Source: @material/material-color-utilities 0.4.0, Material 3 Tonal Spot
