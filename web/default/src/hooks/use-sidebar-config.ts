...
/**
 * Check whether the platform enables a module. Historical personal sidebar
 * preferences no longer control navigation visibility.
 */
...
function isNavItemVisible(
  ...
  // Dynamic chat presets follow the platform's chat section and module gates.
...
/**
 * Filter navigation by the authoritative platform SidebarModulesAdmin config.
 * Role and permission checks remain with the navigation consumers and routes.
 * Personal sidebar preferences are intentionally ignored now that users can no
 * longer configure them, so historical values cannot hide allowed entries.
 */
...
/**
 * Apply the same platform configuration to single-route navigation entries.
 */
