  ...
  return (
    <div className='flex h-8 items-end gap-1' aria-hidden='true'>
      {sparkline.map((height) => {
        const occurrence = occurrences.get(height) ?? 0
        occurrences.set(height, occurrence + 1)

        return (
          <span
            key={`${height}-${occurrence}`}
            className={cn(
              'flex-1 rounded-t-sm',
              height <= 0 && 'opacity-20',
...
export function StatCard(props: StatCardProps) {
  ...
  let supportingContent = <BarSparkline values={props.sparkline} tone={tone} />
  if (props.details?.length) {
    supportingContent = <StatCardDetails details={props.details} />
  } else if (sparklineVariant === 'line') {
    supportingContent = <LineSparkline values={props.sparkline} tone={tone} />
  }
