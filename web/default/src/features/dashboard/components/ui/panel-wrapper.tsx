...
function PanelHeader(props: { title: ReactNode; actions?: ReactNode }) {
  const heading = <div className='text-sm font-semibold'>{props.title}</div>
...
export function PanelWrapper(props: PanelWrapperProps) {
  ...
  return (
    <div className={frameClassName}>
      <PanelHeader title={props.title} actions={props.headerActions} />
      <div className={cn('p-4 sm:p-5', props.contentClassName)}>
        {props.children}
      </div>
    </div>
