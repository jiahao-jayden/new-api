...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        'group/card bg-card text-card-foreground flex flex-col gap-5 overflow-hidden rounded-2xl py-5 text-sm has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-4 data-[size=sm]:py-4 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl',
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'bg-surface-container-low flex items-center rounded-b-2xl p-5 group-data-[size=sm]/card:p-4',
        className
      )}
      {...props}
    />
