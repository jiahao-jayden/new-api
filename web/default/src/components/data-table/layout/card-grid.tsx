  ...
  return (
    <div className={props.gridClassName ?? DEFAULT_GRID_CLASSNAME}>
      {[1, 2, 3, 4, 5, 6].map((i) => (
        <div
          key={`${prefix}-${i}`}
          className='bg-card space-y-3 rounded-2xl p-4'
        >
      ...
      {rows.map((row) => {
        ...
        return (
          <div
            ...
            className={cn(
              'bg-card rounded-2xl px-4 py-3.5 transition-[background-color,box-shadow] duration-150 data-[state=selected]:bg-accent/45 data-[state=selected]:ring-2 data-[state=selected]:ring-primary/30',
