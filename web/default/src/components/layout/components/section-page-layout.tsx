...
export function SectionPageLayout(props: SectionPageLayoutProps) {
  ...
  return (
    <PageFooterProvider container={footerContainer}>
      <Main>
        <div className='pencil-page-toolbar shrink-0 px-3 pt-3 pb-2.5 sm:px-4 sm:pt-4 sm:pb-3'>
          {breadcrumb != null && (
            <div className='mb-2 sm:mb-3'>{breadcrumb}</div>
          )}
          <div className='flex flex-wrap items-center justify-between gap-x-3 gap-y-2 sm:gap-x-4'>
            <div className='min-w-0 flex-1'>
              <h2 className='truncate text-sm font-medium tracking-wide'>
                {title}
              </h2>
            </div>
            {actions != null && (
        ...
        <div
          ...
          className='console-page-footer bg-background shrink-0 border-t px-3 py-2.5 empty:hidden sm:px-4 sm:py-3'
        />
      </Main>
    </PageFooterProvider>
  )
